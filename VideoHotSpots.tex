...
In this demo paper we have presented an approach for discovering Hot Spot in informative and educational Web videos that Leverage not only on a visual analysis of the video itself but also on the knowledge available on Web in order to detect. The presence of this hot spots can lead to an easy decision, an a more user tailored consumption in the form of video fragments. Those hot spots are displayed in a nice Web compliant media player support for semantic annotations. 
Regarding the future, the an exhaustive evaluation involving user feedback in order to tune the results from the Hot Spot generation operation and improves the interaction when manipulating them in the player. In future development we plan to use the segmentation results and their corresponding annotations to establish links between the media content, so the user can easily jump from one particular fragment to other where similar things are being told. 
